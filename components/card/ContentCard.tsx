...
const ContentCard = ({
  ...
  return (
    <div
      className={`w-full rounded-xl h-72 ${height ? "2xl:h-52" : "lg:h-72 2xl:h-72"}  ${
        isCustomBg
    ...
    >
      ...
      >
        <div className="text-white md:w-full  2xl:ml-5 mt-5 md:mt-8 xl:mt-4 text-3xl flex flex-col">
          <span className={` ${widthFull ? "w-full text-2xl" : "w-44"}`}>{title}</span>
          <span className={`text-sm mt-2 ml-1 opacity-95  ${widthFull ? "w-full md:w-[35%] mt-5" : "w-56"}`}>{subtitle}</span>
        ...
        >
          {!hideButton && (
            <Button
              customClass={`px-14 py-2 ${margemTop ? "mt-4" : "mt-0"} ${

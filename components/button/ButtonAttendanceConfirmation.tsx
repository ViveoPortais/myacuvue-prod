...
const ButtonAttendanceConfirmation = ({
  ...
  const handleConfirmation = () => {
    ...
    confirmVisitAttendance(idVisit as any).then((response) => {
  ...
  const handleCancelClick = () => {
    ...
    patientNotAttended(idVisit as any).then((response) => {
      dataScheduling.setRefresh(!dataScheduling.refresh);
  ...
  return (
    <>
      {params?.row?.attendanceStatus !== "Comparecimento cancelado" ? (
        ...
        ${
          params?.row?.attendanceStatus === "Não compareceu"
            ? "bg-careRedButton border-careRedButton w-36 md:w-32 py-2 text-sm"
            : "bg-careGreen border-careGreen w-36 md:w-32 py-2 text-sm"
        }
      `}
          type={type}
        >
      ...
      {params?.row?.attendanceStatus !== "Comparecimento cancelado" ? (
        <Switch
          id="switch-attendance-confirmation"
          {...labelSwift}
